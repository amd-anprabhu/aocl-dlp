...
static inline __declspec(naked) void
__cpuid(int[4], int)
{
    __asm {
				push	ebx
...
#include <cpuid.h>
    ...
    && defined(XBYAK32) // avoid err : can't find a register in class `BREG'
                        // while reloading `asm'
...
namespace Xbyak { namespace util {
    ...
    typedef CpuTopologyLevel
        IntelCpuTopologyLevel; // for backward compatibility
    ...
    class Cpu
    {
      public:
        class Type
        {
          ...
          public:
            Type(uint64_t L = 0, uint64_t H = 0)
                : L(L)
                , H(H)
            {
            }
            template<uint64_t L_, uint64_t H_>
            Type(local::TypeT<L_, H_>)
                : L(L_)
                , H(H_)
            {
            }
      ...
      private:
        ...
        // [EBX:ECX:EDX] == s?
        bool isEqualStr(uint32_t   EBX,
                        uint32_t   ECX,
                        uint32_t   EDX,
                        const char s[12]) const
        {
            return get32bitAsBE(&s[0]) == EBX && get32bitAsBE(&s[4]) == EDX
                   && get32bitAsBE(&s[8]) == ECX;
        }
        ...
        void setNumCores()
        {
            ...
            if (data[0] >= 0xB) {
                ...
                if (data[0] != 0 || data[1] != 0) {
                    /*
                        ...
                        we use it to get the number of smt cores and cores on
                       socket
                    ...
                    */
                    for (uint32_t i = 0; i < maxTopologyLevels; i++) {
                        ...
                        CpuTopologyLevel level =
                            (CpuTopologyLevel)extractBit(data[2], 8, 15);
                        if (level == SmtLevel || level == CoreLevel) {
                            numCores_[level - 1] = extractBit(data[1], 0, 15);
                        }
                    }
                    /*
                        Fallback values in case a hypervisor has the leaf
                       zeroed-out.
                    */
                    numCores_[SmtLevel - 1] =
                        local::max_(1u, numCores_[SmtLevel - 1]);
                    numCores_[CoreLevel - 1] = local::max_(
                        numCores_[SmtLevel - 1], numCores_[CoreLevel - 1]);
                    return;
                }
            }
            ...
            if (has(tAMD)) {
                ...
                */
                ...
                int htt =
                    extractBit(data[3], 28, 28); // Hyper-threading technology.
                ...
                */
                ...
                if (htt == 0) {
                    ...
                    numCores_[SmtLevel - 1] = 1;
                    numCores_[CoreLevel - 1] =
                        logicalProcessorCount > 0 ? logicalProcessorCount : 1;
                }
            }
        }
        void setCacheHierarchy()
        {
            ...
            if (has(tAMD)) {
                ...
                if (data[0] >= 0x8000001D) {
                    // For modern AMD CPUs.
                    dataCacheLevels_ = 0;
                    for (uint32_t subLeaf = 0;
                         dataCacheLevels_ < maxNumberCacheLevels; subLeaf++) {
                        ...
                        */
                        ...
                        if (subLeaf > 0) {
                            numSharingCache =
                                local::min_(numSharingCache, (int)numCores_[1]);
                            numSharingCache /=
                                local::max_(1u, coresSharingDataCache_[0]);
                        }
                        coresSharingDataCache_[dataCacheLevels_] =
                            numSharingCache;
                        dataCacheLevels_ += 1;
                    }
                    coresSharingDataCache_[0] =
                        local::min_(1u, coresSharingDataCache_[0]);
                ...
                */
                for (int i = 0; dataCacheLevels_ < maxNumberCacheLevels; i++) {
                    ...
                    if (cacheType == DATA_CACHE || cacheType == UNIFIED_CACHE) {
                        uint32_t actual_logical_cores =
                            extractBit(data[0], 14, 25) + 1;
                        if (logical_cores != 0) { // true only if leaf 0xB is
                                                  // supported and valid
                            actual_logical_cores = local::min_(
                                actual_logical_cores, logical_cores);
                        }
      ...
      public:
        ...
        */
        static inline void getCpuidEx(uint32_t eaxIn,
                                      uint32_t ecxIn,
                                      uint32_t data[4])
        {
#ifdef XBYAK_INTEL_CPU_SPECIFIC
...
#define XBYAK_DEFINE_TYPE(id, NAME)                                            \
    static const local::TypeT<XBYAK_SPLIT_ID(id)> NAME
        ...
        XBYAK_DEFINE_TYPE(
            22, tBMI2); // bzhi, mulx, pdep, pext, rorx, sarx, shlx, shrx
...
#undef XBYAK_DEFINE_TYPE
            ...
            , avx10version_(0)
        {
            ...
            if (type_ & tOSXSAVE) {
                ...
                if ((bv & 6) == 6) {
                    ...
                    if (ECX & (1U << 28))
                        type_ |= tAVX;
                    // do *not* check AVX-512 state on macOS because it has
                    // on-demand AVX-512 support
#if !defined(__APPLE__)
                    if (((bv >> 5) & 7) == 7)
#endif
                    {
...
#endif
#else
            // TODO: Need another impl of Clock or rdtsc-equivalent for non-x86
            // cpu
            return 0;
#endif
        }
    ...
    class Pack
    {
      ...
      public:
        ...
        Pack(const Xbyak::Reg64& tb,
             const Xbyak::Reg64& ta,
             const Xbyak::Reg64& t9,
             const Xbyak::Reg64& t8,
             const Xbyak::Reg64& t7,
             const Xbyak::Reg64& t6,
    ...
    class StackFrame
    {
      ...
      public:
        ...
        */
        StackFrame(Xbyak::CodeGenerator* code,
                   int                   pNum,
                   int                   tNum          = 0,
                   int                   stackSizeByte = 0,
            ...
            , t(t_)
        {
            ...
            if (P_ > 0 && (P_ & 1) == (saveNum_ & 1))
                P_++; // (rsp % 16) == 8, then increment P_ for 16 byte
                      // alignment
            P_ *= 8;
            if (P_ > 0)
                code->sub(_rsp, P_);
      ...
      public:
        ...
        void init(int mode)
        {
            ...
            switch (mode) {
                ...
                case VTune:
#ifdef XBYAK_USE_VTUNE
                    dlopen("dummy", RTLD_LAZY); // force to load dlopen to
                 ...
                 size_t      funcSize) const
        {
            ...
            if (mode_ == Perf) {
                if (fp_ == 0)
                    return;
                fprintf(fp_, "%llx %zx %s%s", (long long)startAddr, funcSize,
                        funcName, suffix_);
                /*
                    perf does not recognize the function name which is less than
                   3, so append '_' at the end of the name if necessary
                */
